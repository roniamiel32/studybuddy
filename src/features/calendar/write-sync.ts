 ...
 *
 * Modifications:
 *     0.49.0 - 2026-09-01 - Disabled. The whole module is commented out and its
 *                           callers with it; a session reaches a calendar
 *                           through the manual link in the session dialog now
...
/*
 * ============================================================================
 * DISABLED — 2026-09-01. NOTHING IN THIS FILE RUNS.
 *
 * The app no longer pushes anything into a student's Google Calendar. Direct
 * writes need the restricted `calendar.events` scope, which is what drags the
 * project into Google's OAuth verification; the manual
 * `calendar.google.com/calendar/render?action=TEMPLATE` link in the session
 * dialog does the same job for the student and asks Google for nothing.
 *
 * Kept verbatim rather than deleted, so restoring the integration is four
 * uncomments and no rewriting:
 *
 *   1. this file,
 *   2. `createEvent` / `deleteEvent` and the `calendar.events` scope in
 *      src/lib/google/calendar.ts,
 *   3. the import and the four call sites in src/features/meetings/actions.ts,
 *   4. the import and the reconcile call in src/features/calendar/read-sync.ts.
 *
 * `calendar_event_links` and its migrations are untouched, so any rows written
 * before today still point at the events they were written for.
 * ============================================================================
 */

/* The file must stay a module: `isolatedModules` refuses one with no imports
   and no exports, and every real export below is commented out. */
export {};
