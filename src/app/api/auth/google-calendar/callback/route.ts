 ...
 *
 *              COMMENTED OUT IN FULL — 2026-09-01. Nothing redirects here any
 *              more, and nothing may: this is the endpoint that accepted an
 *              authorization code and turned it into stored tokens.
 * Version:     0.50.0
 *
 * Modifications:
 ...
 *
 * NO HANDLER IS EXPORTED, so the path answers 405 Method Not Allowed and no
 * authorization code is ever read — verified against a production build. Next
 * still lists the route because the file exists; what it no longer does is
 * accept a credential.
 *
 * Restoring it is uncommenting this file; the redirect URI registered in Google
 * Cloud never changed, so nothing outside the repo needs touching.
 */
