 ...
 *
 * `calendar.readonly` would be enough for the read sync alone, but the write
 * sync creates and deletes events, and `calendar.events` is the narrowest scope
 * that permits it. `userinfo.email` exists only so the UI can name the connected
 * account — a student with a personal and a university Google account otherwise
 * has no way to tell which one they attached.
 */
