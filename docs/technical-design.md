...
| `created_at` | `timestamptz` | not null, default `now()` |
| | | `unique (profile_id, day_of_week, starts_at)` |
...
create or replace function app_are_connected(a uuid, b uuid)
returns boolean language sql stable security definer set search_path = public as $$
  select exists (
    select 1 from connection_requests r
    where r.status = 'accepted'
      and ((r.requester_id = a and r.addressee_id = b)
        or (r.requester_id = b and r.addressee_id = a))
  );
$$;
...
| `profile_contacts` | self, **or** `app_are_connected(auth.uid(), profile_id)` | self | self | self |
...
│   ├── config.toml
│   ├── migrations/
│   │   ├── 20260803_0100_enums.sql
│   │   ├── 20260803_0200_tenancy.sql
│   │   ├── 20260803_0300_profiles.sql
│   │   ├── 20260803_0400_academic.sql
│   │   ├── 20260803_0500_connections.sql
│   │   ├── 20260803_0600_ai_tables.sql
│   │   ├── 20260803_0700_functions_triggers.sql
│   │   ├── 20260803_0800_rls.sql
│   │   └── 20260803_0900_rpc_find_candidates.sql
│   └── seed/
│       ├── 01_universities.sql
│       └── 02_runi_2026b_courses.sql
├── src/
...
| `saveAvailability` | `(slots: SlotInput[]) → ActionResult<void>` — merges overlapping slots server-side, replaces the week in one transaction |
...
4a → 3b → 3c → 4b. That sequence gives a **complete, demonstrable product
without any AI** by 4a, and treats the AI as the enhancement the PRD says it
is. Cutting 3b/3c then costs you the "smart" story but not a working app.
